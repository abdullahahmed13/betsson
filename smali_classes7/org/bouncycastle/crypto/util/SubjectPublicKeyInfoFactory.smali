.class public Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;
.super Ljava/lang/Object;


# static fields
.field private static cryptoProOids:Ljava/util/Set;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    new-instance v0, Ljava/util/HashSet;

    const/4 v1, 0x5

    invoke-direct {v0, v1}, Ljava/util/HashSet;-><init>(I)V

    sput-object v0, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->cryptoProOids:Ljava/util/Set;

    sget-object v1, Lorg/bouncycastle/asn1/cryptopro/a;->x:Lorg/bouncycastle/asn1/n;

    invoke-interface {v0, v1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    sget-object v0, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->cryptoProOids:Ljava/util/Set;

    sget-object v1, Lorg/bouncycastle/asn1/cryptopro/a;->y:Lorg/bouncycastle/asn1/n;

    invoke-interface {v0, v1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    sget-object v0, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->cryptoProOids:Ljava/util/Set;

    sget-object v1, Lorg/bouncycastle/asn1/cryptopro/a;->z:Lorg/bouncycastle/asn1/n;

    invoke-interface {v0, v1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    sget-object v0, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->cryptoProOids:Ljava/util/Set;

    sget-object v1, Lorg/bouncycastle/asn1/cryptopro/a;->A:Lorg/bouncycastle/asn1/n;

    invoke-interface {v0, v1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    sget-object v0, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->cryptoProOids:Ljava/util/Set;

    sget-object v1, Lorg/bouncycastle/asn1/cryptopro/a;->B:Lorg/bouncycastle/asn1/n;

    invoke-interface {v0, v1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    return-void
.end method

.method private constructor <init>()V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static createSubjectPublicKeyInfo(Lorg/bouncycastle/crypto/params/AsymmetricKeyParameter;)Lorg/bouncycastle/asn1/x509/d;
    .locals 10

    instance-of v0, p0, Lorg/bouncycastle/crypto/params/RSAKeyParameters;

    if-eqz v0, :cond_0

    check-cast p0, Lorg/bouncycastle/crypto/params/RSAKeyParameters;

    new-instance v0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v1, Lorg/bouncycastle/asn1/x509/a;

    sget-object v2, Lorg/bouncycastle/asn1/pkcs/b;->b:Lorg/bouncycastle/asn1/n;

    sget-object v3, Lorg/bouncycastle/asn1/t0;->c:Lorg/bouncycastle/asn1/t0;

    invoke-direct {v1, v2, v3}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;Lorg/bouncycastle/asn1/e;)V

    new-instance v2, Lorg/bouncycastle/asn1/pkcs/f;

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/RSAKeyParameters;->getModulus()Ljava/math/BigInteger;

    move-result-object v3

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/RSAKeyParameters;->getExponent()Ljava/math/BigInteger;

    move-result-object p0

    invoke-direct {v2, v3, p0}, Lorg/bouncycastle/asn1/pkcs/f;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;)V

    invoke-direct {v0, v1, v2}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;Lorg/bouncycastle/asn1/e;)V

    return-object v0

    :cond_0
    instance-of v0, p0, Lorg/bouncycastle/crypto/params/DSAPublicKeyParameters;

    const/4 v1, 0x0

    if-eqz v0, :cond_2

    check-cast p0, Lorg/bouncycastle/crypto/params/DSAPublicKeyParameters;

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/DSAKeyParameters;->getParameters()Lorg/bouncycastle/crypto/params/DSAParameters;

    move-result-object v0

    if-eqz v0, :cond_1

    new-instance v1, Lorg/bouncycastle/asn1/x509/b;

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/DSAParameters;->getP()Ljava/math/BigInteger;

    move-result-object v2

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/DSAParameters;->getQ()Ljava/math/BigInteger;

    move-result-object v3

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/DSAParameters;->getG()Ljava/math/BigInteger;

    move-result-object v0

    invoke-direct {v1, v2, v3, v0}, Lorg/bouncycastle/asn1/x509/b;-><init>(Ljava/math/BigInteger;Ljava/math/BigInteger;Ljava/math/BigInteger;)V

    :cond_1
    new-instance v0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v2, Lorg/bouncycastle/asn1/x509/a;

    sget-object v3, Lorg/bouncycastle/asn1/x9/n;->y0:Lorg/bouncycastle/asn1/n;

    invoke-direct {v2, v3, v1}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;Lorg/bouncycastle/asn1/e;)V

    new-instance v1, Lorg/bouncycastle/asn1/k;

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/DSAPublicKeyParameters;->getY()Ljava/math/BigInteger;

    move-result-object p0

    invoke-direct {v1, p0}, Lorg/bouncycastle/asn1/k;-><init>(Ljava/math/BigInteger;)V

    invoke-direct {v0, v2, v1}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;Lorg/bouncycastle/asn1/e;)V

    return-object v0

    :cond_2
    instance-of v0, p0, Lorg/bouncycastle/crypto/params/ECPublicKeyParameters;

    if-eqz v0, :cond_8

    check-cast p0, Lorg/bouncycastle/crypto/params/ECPublicKeyParameters;

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/ECKeyParameters;->getParameters()Lorg/bouncycastle/crypto/params/ECDomainParameters;

    move-result-object v0

    const/4 v2, 0x0

    if-nez v0, :cond_3

    new-instance v0, Lorg/bouncycastle/asn1/x9/f;

    sget-object v1, Lorg/bouncycastle/asn1/t0;->c:Lorg/bouncycastle/asn1/t0;

    invoke-direct {v0, v1}, Lorg/bouncycastle/asn1/x9/f;-><init>(Lorg/bouncycastle/asn1/l;)V

    goto/16 :goto_1

    :cond_3
    instance-of v3, v0, Lorg/bouncycastle/crypto/params/ECGOST3410Parameters;

    if-eqz v3, :cond_6

    check-cast v0, Lorg/bouncycastle/crypto/params/ECGOST3410Parameters;

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/ECPublicKeyParameters;->getQ()Lorg/bouncycastle/math/ec/i;

    move-result-object v3

    invoke-virtual {v3}, Lorg/bouncycastle/math/ec/i;->e()Lorg/bouncycastle/math/ec/f;

    move-result-object v3

    invoke-virtual {v3}, Lorg/bouncycastle/math/ec/f;->t()Ljava/math/BigInteger;

    move-result-object v3

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/ECPublicKeyParameters;->getQ()Lorg/bouncycastle/math/ec/i;

    move-result-object p0

    invoke-virtual {p0}, Lorg/bouncycastle/math/ec/i;->f()Lorg/bouncycastle/math/ec/f;

    move-result-object p0

    invoke-virtual {p0}, Lorg/bouncycastle/math/ec/f;->t()Ljava/math/BigInteger;

    move-result-object p0

    new-instance v4, Lorg/bouncycastle/asn1/cryptopro/c;

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECGOST3410Parameters;->getPublicKeyParamSet()Lorg/bouncycastle/asn1/n;

    move-result-object v5

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECGOST3410Parameters;->getDigestParamSet()Lorg/bouncycastle/asn1/n;

    move-result-object v6

    invoke-direct {v4, v5, v6}, Lorg/bouncycastle/asn1/cryptopro/c;-><init>(Lorg/bouncycastle/asn1/n;Lorg/bouncycastle/asn1/n;)V

    sget-object v5, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->cryptoProOids:Ljava/util/Set;

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECGOST3410Parameters;->getPublicKeyParamSet()Lorg/bouncycastle/asn1/n;

    move-result-object v0

    invoke-interface {v5, v0}, Ljava/util/Set;->contains(Ljava/lang/Object;)Z

    move-result v0

    const/16 v5, 0x20

    const/16 v6, 0x40

    if-eqz v0, :cond_4

    sget-object v0, Lorg/bouncycastle/asn1/cryptopro/a;->m:Lorg/bouncycastle/asn1/n;

    goto :goto_0

    :cond_4
    invoke-virtual {v3}, Ljava/math/BigInteger;->bitLength()I

    move-result v0

    const/16 v7, 0x100

    if-le v0, v7, :cond_5

    sget-object v0, Lorg/bouncycastle/asn1/rosstandart/a;->h:Lorg/bouncycastle/asn1/n;

    const/16 v5, 0x80

    move v9, v6

    move v6, v5

    move v5, v9

    goto :goto_0

    :cond_5
    sget-object v0, Lorg/bouncycastle/asn1/rosstandart/a;->g:Lorg/bouncycastle/asn1/n;

    :goto_0
    new-array v7, v6, [B

    div-int/lit8 v6, v6, 0x2

    invoke-static {v7, v6, v2, v3}, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->extractBytes([BIILjava/math/BigInteger;)V

    invoke-static {v7, v6, v5, p0}, Lorg/bouncycastle/crypto/util/SubjectPublicKeyInfoFactory;->extractBytes([BIILjava/math/BigInteger;)V

    :try_start_0
    new-instance p0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v2, Lorg/bouncycastle/asn1/x509/a;

    invoke-direct {v2, v0, v4}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;Lorg/bouncycastle/asn1/e;)V

    new-instance v0, Lorg/bouncycastle/asn1/v0;

    invoke-direct {v0, v7}, Lorg/bouncycastle/asn1/v0;-><init>([B)V

    invoke-direct {p0, v2, v0}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;Lorg/bouncycastle/asn1/e;)V
    :try_end_0
    .catch Ljava/io/IOException; {:try_start_0 .. :try_end_0} :catch_0

    return-object p0

    :catch_0
    return-object v1

    :cond_6
    instance-of v1, v0, Lorg/bouncycastle/crypto/params/ECNamedDomainParameters;

    if-eqz v1, :cond_7

    new-instance v1, Lorg/bouncycastle/asn1/x9/f;

    check-cast v0, Lorg/bouncycastle/crypto/params/ECNamedDomainParameters;

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECNamedDomainParameters;->getName()Lorg/bouncycastle/asn1/n;

    move-result-object v0

    invoke-direct {v1, v0}, Lorg/bouncycastle/asn1/x9/f;-><init>(Lorg/bouncycastle/asn1/n;)V

    move-object v0, v1

    goto :goto_1

    :cond_7
    new-instance v3, Lorg/bouncycastle/asn1/x9/h;

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECDomainParameters;->getCurve()Lorg/bouncycastle/math/ec/e;

    move-result-object v4

    new-instance v5, Lorg/bouncycastle/asn1/x9/j;

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECDomainParameters;->getG()Lorg/bouncycastle/math/ec/i;

    move-result-object v1

    invoke-direct {v5, v1, v2}, Lorg/bouncycastle/asn1/x9/j;-><init>(Lorg/bouncycastle/math/ec/i;Z)V

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECDomainParameters;->getN()Ljava/math/BigInteger;

    move-result-object v6

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECDomainParameters;->getH()Ljava/math/BigInteger;

    move-result-object v7

    invoke-virtual {v0}, Lorg/bouncycastle/crypto/params/ECDomainParameters;->getSeed()[B

    move-result-object v8

    invoke-direct/range {v3 .. v8}, Lorg/bouncycastle/asn1/x9/h;-><init>(Lorg/bouncycastle/math/ec/e;Lorg/bouncycastle/asn1/x9/j;Ljava/math/BigInteger;Ljava/math/BigInteger;[B)V

    new-instance v0, Lorg/bouncycastle/asn1/x9/f;

    invoke-direct {v0, v3}, Lorg/bouncycastle/asn1/x9/f;-><init>(Lorg/bouncycastle/asn1/x9/h;)V

    :goto_1
    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/ECPublicKeyParameters;->getQ()Lorg/bouncycastle/math/ec/i;

    move-result-object p0

    invoke-virtual {p0, v2}, Lorg/bouncycastle/math/ec/i;->j(Z)[B

    move-result-object p0

    new-instance v1, Lorg/bouncycastle/asn1/x509/d;

    new-instance v2, Lorg/bouncycastle/asn1/x509/a;

    sget-object v3, Lorg/bouncycastle/asn1/x9/n;->G:Lorg/bouncycastle/asn1/n;

    invoke-direct {v2, v3, v0}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;Lorg/bouncycastle/asn1/e;)V

    invoke-direct {v1, v2, p0}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;[B)V

    return-object v1

    :cond_8
    instance-of v0, p0, Lorg/bouncycastle/crypto/params/X448PublicKeyParameters;

    if-eqz v0, :cond_9

    check-cast p0, Lorg/bouncycastle/crypto/params/X448PublicKeyParameters;

    new-instance v0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v1, Lorg/bouncycastle/asn1/x509/a;

    sget-object v2, Lorg/bouncycastle/asn1/edec/a;->c:Lorg/bouncycastle/asn1/n;

    invoke-direct {v1, v2}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;)V

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/X448PublicKeyParameters;->getEncoded()[B

    move-result-object p0

    invoke-direct {v0, v1, p0}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;[B)V

    return-object v0

    :cond_9
    instance-of v0, p0, Lorg/bouncycastle/crypto/params/X25519PublicKeyParameters;

    if-eqz v0, :cond_a

    check-cast p0, Lorg/bouncycastle/crypto/params/X25519PublicKeyParameters;

    new-instance v0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v1, Lorg/bouncycastle/asn1/x509/a;

    sget-object v2, Lorg/bouncycastle/asn1/edec/a;->b:Lorg/bouncycastle/asn1/n;

    invoke-direct {v1, v2}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;)V

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/X25519PublicKeyParameters;->getEncoded()[B

    move-result-object p0

    invoke-direct {v0, v1, p0}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;[B)V

    return-object v0

    :cond_a
    instance-of v0, p0, Lorg/bouncycastle/crypto/params/Ed448PublicKeyParameters;

    if-eqz v0, :cond_b

    check-cast p0, Lorg/bouncycastle/crypto/params/Ed448PublicKeyParameters;

    new-instance v0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v1, Lorg/bouncycastle/asn1/x509/a;

    sget-object v2, Lorg/bouncycastle/asn1/edec/a;->e:Lorg/bouncycastle/asn1/n;

    invoke-direct {v1, v2}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;)V

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/Ed448PublicKeyParameters;->getEncoded()[B

    move-result-object p0

    invoke-direct {v0, v1, p0}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;[B)V

    return-object v0

    :cond_b
    instance-of v0, p0, Lorg/bouncycastle/crypto/params/Ed25519PublicKeyParameters;

    if-eqz v0, :cond_c

    check-cast p0, Lorg/bouncycastle/crypto/params/Ed25519PublicKeyParameters;

    new-instance v0, Lorg/bouncycastle/asn1/x509/d;

    new-instance v1, Lorg/bouncycastle/asn1/x509/a;

    sget-object v2, Lorg/bouncycastle/asn1/edec/a;->d:Lorg/bouncycastle/asn1/n;

    invoke-direct {v1, v2}, Lorg/bouncycastle/asn1/x509/a;-><init>(Lorg/bouncycastle/asn1/n;)V

    invoke-virtual {p0}, Lorg/bouncycastle/crypto/params/Ed25519PublicKeyParameters;->getEncoded()[B

    move-result-object p0

    invoke-direct {v0, v1, p0}, Lorg/bouncycastle/asn1/x509/d;-><init>(Lorg/bouncycastle/asn1/x509/a;[B)V

    return-object v0

    :cond_c
    new-instance p0, Ljava/io/IOException;

    const-string v0, "key parameters not recognized"

    invoke-direct {p0, v0}, Ljava/io/IOException;-><init>(Ljava/lang/String;)V

    throw p0
.end method

.method private static extractBytes([BIILjava/math/BigInteger;)V
    .locals 4

    invoke-virtual {p3}, Ljava/math/BigInteger;->toByteArray()[B

    move-result-object p3

    array-length v0, p3

    const/4 v1, 0x0

    if-ge v0, p1, :cond_0

    new-array v0, p1, [B

    array-length v2, p3

    sub-int v2, p1, v2

    array-length v3, p3

    invoke-static {p3, v1, v0, v2, v3}, Ljava/lang/System;->arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V

    move-object p3, v0

    :cond_0
    :goto_0
    if-eq v1, p1, :cond_1

    add-int v0, p2, v1

    array-length v2, p3

    add-int/lit8 v2, v2, -0x1

    sub-int/2addr v2, v1

    aget-byte v2, p3, v2

    aput-byte v2, p0, v0

    add-int/lit8 v1, v1, 0x1

    goto :goto_0

    :cond_1
    return-void
.end method
